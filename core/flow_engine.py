            ...
            if node_type == NodeType.DELAY:
                if "random_ms" in node:
                    ...
                    if not isinstance(random_ms, (list, tuple)) or len(random_ms) != 2:
                        raise ValueError("random_ms must be a list/tuple with exactly 2 numeric values [min_ms, max_ms]")
                    try:
                    ...
                    if min_ms > max_ms:
                        raise ValueError(f"random_ms values invalid: min_ms ({min_ms}) must be <= max_ms ({max_ms})")
